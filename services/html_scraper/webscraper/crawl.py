...
def crawl(settings={}, spider_name="vanassem", spider_kwargs={}):
    ...
    try:
        spider_key = urlparse(spider_kwargs.get("start_urls")[0]).hostname if spider_kwargs.get("start_urls") else urlparse(spider_cls.start_urls[0]).hostname
    except Exception:
